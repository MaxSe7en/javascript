...
function fast3FantanResults(drawNumber, game, optionalBallPosition = 1) {
  ...
  switch (game.toLowerCase()) {
    ...
    case "short deck dice": {
      ...
      GAME_RESULTS.push(
        `${SUM_OF_THREE}`,
        sPair([num1, num2, num3])[0] ? sPair([num1, num2, num3])[1].join(",") : "",
        hasTriple([num1, num2, num3]) ? DRAW_NUMBERS.map(String).join(",") : ""
      );
      break;
    }
    case "miscellaneous long deck": {
      GAME_RESULTS.push(...generateAllPairs(DRAW_NUMBERS).map(String));
      break;
    }
    case "ball position": {
      const ballPositionObject = positionOfBallNode(DRAW_NUMBERS);
...
console.log(
  ...
  // sPair([0, 0, 2])
  fast3FantanResults([2, 4, 1], "Miscellaneous Long Deck", 2)
);
